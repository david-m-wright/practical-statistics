...
The data `covid` always comes first and then it is piped `%>%` into `ggplot()`, as the first argument of `ggplot()` is always the data to be plotted. Next comes the aesthetic mapping `aes()` which is used to define which variables will be plotted on the x and y axes. Finally, as this is to be a line chart, `geom_line()` is used to produce the chart. Once a ggplot has been defined, all subsequent _layers_ (geoms) are added using a trailing `+` at the end of a row, not the pipe operator.
